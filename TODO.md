...
- [ ] Измеримый eval: 10–20 прошлых PR (с дефектами, чистые, крупные), метрики @owner:github:andrei-shtanakov @id:review-kit-eval-harness
  ...
  **Состав знаменателя `blocking_recall` — три дефекта, и его надо называть, а
  ...
  проверкой, а не допущением (находка приёмочного dry-run на PR #170,
  minor/medium): в свежем пустом bare-репозитории, без локального чекаута,
  `git fetch --depth 1 <remote_url> <sha>` — тот самый путь, которым идёт
  `cache.py`, — отдаёт обе головы. Отказ `corpus materialize` на чистой машине
  не наступает, и единственный дефект вне прозы из знаменателя не выпадает.
  Пункт `review-eval-prefix-heads` остаётся открытым как общее правило для
  будущих кейсов, но эти два от него не блокированы.
