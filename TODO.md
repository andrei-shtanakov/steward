...
- [x] Приём steward#181 (from devtools#260, их PR #281 в черновике): в ките
      ...
      **закрыт PR этой ветки** по двум из трёх пунктов DoD заявки (третий,
      ...
      безусловный дисклеймер, не отличавший свежую границу от протухшей.
      Теперь называется результат; remote берётся из полного имени ref'а
      (`refs/remotes/<remote>/<ветка>`), а не угадывается, поэтому проверка
      работает и при `--base <sha>`, где резолв remote'а для базы не
      исполнялся вовсе. Фетча нет намеренно: что и когда тянуть, знает
      вызывающий — он границу и назвал.
