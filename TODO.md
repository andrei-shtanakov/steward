...
- [ ] Дедуп ревью по снимку диффа — вердикт не перегоняется на байт-идентичном входе @owner:github:andrei-shtanakov @id:review-dedup-diff-hash @blocked_by:todo://devtools/review-pr-fp-slurp-compat
  ...
  верное, экономия нулевая и молчаливая). Дефект — devtools#75, ожидание
  переставлено на него. Остаток после фикса: обкатка на живых PR и замер
  экономии (за steward) — доля унаследованных вердиктов против платных
  прогонов, по маркерам `fp=` в опубликованных ревью.
