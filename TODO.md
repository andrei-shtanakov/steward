...
- [ ] Регулярный сбор `approval-facts` — Stage A0 (steward-only soak) @owner:github:andrei-shtanakov @id:approval-facts-scheduled-collection
      ...
      **Решение о поверхности (2026-08-23).** Раннер защищает ДАННЫЕ и не защищает
      топологию файловой системы: нет проверок на hardlink бандла, на подмену
      `.steward` симлинком во время прогона, на регистровые алиасы путей и на
      разрешение ssh-алиасов. Область определения A0 — один репозиторий, одна машина,
      воркспейс оператора, и чекауты в нём предполагаются не враждебными; допущение
      названо в докстринге раннера, а не подразумевается.
