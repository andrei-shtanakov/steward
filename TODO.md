...
- [ ] dispatcher перевендоривает `contracts/gate-verdicts/v1` с `prev_hash` (dispatcher#173) @owner:repo:dispatcher @blocked_by:dispatcher#gate-verdicts-v1-prev-hash-revendor @id:gate-verdicts-prev-hash-dispatcher-revendor — до ре-вендоринга их панель классифицирует сцепленные файлы unreadable (красное, но fail-closed); признак «сделано» — их copy-integrity зелёная на byte-equal копии
...
- [x] jq-префлайт в `apply-threshold.sh` + доводка комментария прохода 2
      ...
      PR этой ветки: `command -v jq` → код 2 с причиной (по образцу
