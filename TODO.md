...
- [ ] `approval_facts_outcome` возвращает `FactsUnavailable("absent")`, когда чекаут не опознан (нет git/`origin`/`origin` не разбирается), **до** проверки `explicit` — поэтому явно указанный негодный `--approval-facts` вне опознанного репозитория читается как «evidence отсутствует» вместо обещанного §8.3.1 config error, exit 2 @owner:github:andrei-shtanakov @id:approval-facts-explicit-path-subordinate-to-repo-id — найдено Codex-гейтом на PR #86 (раунд 2); направление отказа верное (находка, не пропуск), только не тот класс ошибки; не правится этим пунктом — честно применить §8.3.1 здесь означало бы валидировать явно указанный файл по инварианту 11 без `expected_repository`, с которым сравнивать, а это отдельный вопрос дизайна (что означает инвариант 11 без опознанного репозитория), не правка в конце ветки; докстринг `approval_facts_outcome` (`src/steward/gatecheck/cli.py`) объясняет порядок явно
