...
- [ ] **Разрешение `agent_merge` — значение политики, а не константа кода**: `approval.py` безусловно выдаёт `GC-APPROVAL-MISSING` для актора `agent` («disabled by policy (ADR-ECO-004)»); вынести разрешение в `profiles/approval-policy.yaml` с дефолтом «запрещено» (ADR-ECO-008 в статусе `proposed` — мерж документа ≠ ратификация), внести merge-личность GitHub App в `agent_identities` (хинт `"Bot"` уже даёт `agent`), `unknown` остаётся fail-closed @owner:github:andrei-shtanakov @id:agent-merge-policy-driven — приём входящего steward#69 (from prograph-vault#adr-eco-008); это запрос на готовность, не на включение
