...
- [ ] Дедуп ревью по снимку диффа — вердикт не перегоняется на байт-идентичном входе @owner:github:andrei-shtanakov @id:review-dedup-diff-hash @blocked_by:todo://devtools/review-pr-fp-inherit
  ...
  `review-pr.sh` — запрошена inbox devtools#72 с полным контрактом (маркер
  `fp=`, наследование только из доверенного полностью распарсенного ревью
  ai-prosto, `--fresh` как обход поиска, feature-detect кита целевого репо);
  после её обкатки — замер экономии, он за steward.
