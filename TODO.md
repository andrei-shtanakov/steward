...
- [ ] Generated-фильтр не разбирает кавыченные `diff --git`-заголовки (пути со
      спецсимволами/пробелами): такой блок не становится кандидатом и остаётся
      в дифе — худший исход сегодня это явный отказ по потолку (fail в сторону
      ревью, находка minor гейта на #99, подтверждена шестым заходом). Правка —
      парсинг кавыченной формы в прегейте и awk `build-prompt.sh` плюс
      согласованная нормализация C-квотинга в `--tree-list` (git ls-tree квотит
      те же пути — сырое сравнение путей должно остаться корректным)
