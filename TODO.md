...
- [x] Перевести брокер на канонический исполнитель в зонтике @owner:github:andrei-shtanakov @id:broker-via-reusable — реализация переехала в `ai-orchestrators-workspace` (их PR #17), здесь остался тонкий каллер, побайтово равный шаблону `ci/governance/vendor/merge-broker.yml` с подставленным пином. **I2 теперь выполняется конструкцией, а не самопроверкой:** предусловия лежат в чужом для агента репозитории, и guard-список брокера перестал быть себе свидетелем
