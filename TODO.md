...
- [ ] **Включение D1: `agent_merge_allowed: true` после detection loop** @owner:github:andrei-shtanakov @blocked_by:todo://dispatcher/agent-merge-observability @id:agent-merge-d1-enablement @epic:eco.governance-plane — ожидание включения D1 живёт ЗДЕСЬ (находка гейта на PR #120: закрытие пункта дисциплины стёрло единственный открытый носитель этого @blocked_by). Предусловия выполнены: различимость личности (I4-приёмка 2026-08-20), update-branch-дисциплина (`@id:merge-broker-update-branch-discipline`); не выполнено: detection loop у dispatcher. При снятии блокера: переключить `agent_merge_allowed` в `profiles/approval-policy.yaml` (authority-root, мержит человек), перепроверить довод запрета (`@id:agent-merge-policy-rationale-refresh`), при необходимости — issue зонтику; учесть открытые стыковые дыры `@id:broker-key-exposure-per-repo`, steward#121 (timeout), steward#122 (bot-actor synchronize — для D1 дедлок дисциплины, чинить ДО включения)
...
- [ ] Дедуп ревью по снимку диффа: отпечаток входа = sha256(канонизированный
      `git diff base...head` + промпт + схема + порог); вердикт публикуется с
      отпечатком в маркере (рядом с `head=…`), прогон перед вызовом codex ищет
      вердикт с тем же отпечатком → найден → «вердикт унаследован от прогона N,
      дифф не менялся», вызов пропущен; изменился хоть байт входа (включая
      инструкции кита) → обычное ревью. Довод измерен: 26–28.08 минимум 5–6 из
      ~15 платных CI-прогонов шли по байт-идентичным диффам (rerun после
      инфрафлейка, close/reopen против bot-actor, update-branch брокера); после
      смены дефолта (vault#106) аргумент сильнее — всю нагрузку несёт
      терминальный канал, повторы жгут подписочные лимиты, тот самый ресурс,
      ради которого дефолт менялся. POSIX-sh внутри кита, разъезжается
      re-vendor'ом; терминальная обвязка переиспользует ту же проверку — приём
  ...
  Триггер сработал (владелец, 2026-08-28), дизайн утверждён с поправками.
  Кит-половина сделана PR этой ветки: `local.sh --fingerprint-only` — оффлайн
  ...
  диф — штатное «ревьюировать нечего» без отпечатка. Остаток: интеграция в
  `review-pr.sh` (devtools, inbox после мержа) — маркер `fp=`, наследование
  только из доверенного полностью распарсенного ревью ai-prosto (DISMISSED /
  битый маркер / неизвестный action = промах), `--fresh` как обход поиска,
  feature-detect `--fingerprint-only` в ките целевого репо (нет → полный
  прогон, без ре-вендор-прерогатив); затем замер экономии.
...
- [ ] `checksum.sh`, PIN и watch дрейфа — при первом потребителе кита @owner:github:andrei-shtanakov @id:review-kit-vendoring @epic:eco.codex-review-rollout
