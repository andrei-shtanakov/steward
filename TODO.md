...
- [x] Опубликовать `contracts/approval-facts/v1` в дисциплине `gate-verdicts/v1` (SCHEMA + fixtures + README + PIN), пригодный к вендорингу пиненой копией @owner:github:andrei-shtanakov @id:approval-facts-external-contract — реализовано как `contracts/approval-facts/v2/` (SCHEMA.json + fixtures + README + PIN), все четыре обязательных свойства выполнены; приёмка на реальных мержах — `docs/evidence/2026-08-21-approval-facts-v2-migration/manifest.md`
