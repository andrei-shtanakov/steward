...
строка обязана быть парсируемой, нести `prev_hash` и сходиться с хешом
предыдущей строки. Верификатор — `steward verdicts-verify <файл>` (exit 0
chained|legacy, 1 broken, 2 config error); библиотека —
`steward.verdicts.chain.verify_chain`. Producer steward эмитит цепочку на
каждой записи начиная со строки 2.
