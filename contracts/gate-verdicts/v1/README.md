...
## Целостность: hash-chain (`prev_hash`, steward#105)

Каждая запись после строки 1 несёт `prev_hash` — SHA-256 (hex) от точных
UTF-8-байтов **предыдущей строки** без завершающего перевода строки. Header
(строка 1) — якорь цепочки и поле не несёт никогда: его `$def` в схеме поля не
объявляет, так что «сцепленный header» не проходит валидацию.

**Определение строки — нормативно:** строка = байты между `\n`-разделителями;
`\r`, если он есть, — часть строки, не разделителя. Producer пишет только LF,
поэтому LF→CRLF-конвертация файла — это модификация его байтов, и цепочка
ломается **намеренно**: у тампер-эвидентного леджера нет понятия «безобидной»
перезаписи байтов (решение владельца, steward PR #109; чтение «CRLF должен
проходить» предложено и отклонено). Прочие Unicode-границы строк (U+2028 и
т.п.) разделителями не являются — сырой U+2028 легален внутри JSON-строки и
записи не рвёт.

Правило аддитивности: поле в схеме **опционально** — файлы, выпущенные до его
появления, валидны как legacy; с первой записи, несущей поле, каждая следующая
строка обязана нести `prev_hash`, сходящийся с хешом предыдущей строки. При
этом **каждая** строка файла (и до цепочки) обязана быть парсируемым JSON:
непарсируемая строка — `broken`, никогда не «legacy-валидно» (зеркало правила
header'а схемы: unreadable не бывает clean). Верификатор —
`steward verdicts-verify <файл>` (exit 0 chained|legacy, 1 broken, 2 config
error); библиотека — `steward.verdicts.chain.verify_chain`, которой надо
передавать **сырые байты** (`path.read_bytes()`): `read_text()` в
universal-newlines режиме переписал бы `\r\n` обратно в `\n` ДО проверки и
молча «починил» CRLF-подмену. Верификатор дополнительно требует от строки 1
`kind: header` и `schema_version: "1"` — файл иной версии или без header'а
никогда не отчитывается валидным (глубже схему верификатор не проверяет — это
работа читателя). Producer steward эмитит цепочку на каждой записи начиная со
строки 2.

Что цепочка доказывает — и что честно нет: подмена, правка или перестановка
строки **в середине** леджера ломает `prev_hash` следующей записи и
обнаружима машинно, без внешнего состояния. Усечение **хвоста** и полная
перезапись файла с пересчётом хешей цепочкой не ловятся — для этого нужен
внешний якорь (опубликованная голова цепочки), что сознательно вне scope v1.
...
| `chained.jsonl` | `findings.jsonl` с корректной hash-chain (`prev_hash` со строки 2) | ✅ verdicts-verify: `chained` |
| `broken_chain.jsonl` | строка 4 подменена без пересчёта хеша следующей | ✅ схема молчит **намеренно** — подмену видит только верификатор цепочки: `broken` на строке 5 |
...
Аддитивное изменение (опциональное поле, как `obligation` при активации каталога
и `prev_hash` в steward#105) остаётся в `v1`, но требует **ре-вендоринга** копии
потребителем ДО того, как он встретит новые файлы: его прежняя пиненая копия с
`additionalProperties: false` классифицирует запись с новым полем как
unreadable — fail-closed, но красный. Поэтому аддитивное изменение обязано
сопровождаться inbox-handoff'ом каждому потребителю на ре-вендоринг (ADR-ECO-006),
а не обнаруживаться их вахтой дрейфа постфактум.
