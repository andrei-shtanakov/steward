...
#: Пробел после тире необязателен: при пустом `reason` рендер печатает «— » с
#: хвостовым пробелом, который `strip()` строки стирает, — запись с пустой
#: причиной остаётся записью, а не мусором.
