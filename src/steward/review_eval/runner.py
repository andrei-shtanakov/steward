...
#: `--fetch`/`--remote` тянули бы сеть в офлайн-прогон. `--trusted-base`
#: (steward#181) назван ОТДЕЛЬНОЙ строкой, а не покрыт приставкой `--base`:
#: совпадения приставок нет (`"--trusted-base".startswith("--base")` — ложь),
#: и без своей строки он проходил бы фильтр насквозь, сдвигая кейсу границу
#: доверия — то есть то, ЧТО измеряется. `--include-prose`
