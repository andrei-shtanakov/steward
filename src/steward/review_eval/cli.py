...
def _report(
    ...
    *,
    ...
    try:
        # Все три пути проверяются на симлинк до первой записи: иначе отказ
        # второго или третьего писателя на симлинке, встреченном уже после
        # первого `os.replace`, оставлял бы каталог с артефактами от РАЗНЫХ
        # пересчётов — metrics.json уже новый, report.md ещё старый (ревью-
        # находка части 3, minor). Это конкретный, воспроизводимый класс
        # препятствия (симлинк), не гарантия против любого отказа файловой
        # системы — ENOSPC или неудаляемый обычный файл на месте `.tmp`
        # check_writable не ловит (см. её докстринг), и здесь это не
        # обещается.
