...
def _load_catalog() -> GateCatalog:
    """Load the gate-id catalog from ``profiles/``, same directory ``_resolve_profile`` uses."""
    try:
        return load_catalog(Path("profiles") / "gate-catalog.yaml", Path("profiles") / "roles.yaml")
