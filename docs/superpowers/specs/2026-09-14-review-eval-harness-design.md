...
`base_sha` черновика — **merge-base диапазона ревью**, и она запрашивается у
API, а не копируется из `base.sha` PR. После мержа голова базы содержит сам PR
и может быть потомком `head_sha` из маркера: `local.sh` посчитал бы
merge-base(голова базы, head) = head, и кейс получил бы пустой диапазон — на
вид нормальный, а измерять в нём нечего. Кандидат базы — первый родитель
`merge_commit_sha` (у squash- и merge-коммита это голова базы на момент
мержа), у открытого PR — `base.sha`; сама база —
`compare/<кандидат>...<head>` → `merge_base_commit.sha`. Пустой диапазон
(merge-base == head, либо compare отвечает `identical`/`behind`) — отказ, а не
кейс.
...
6. **Классификация исхода** — по коду выхода **и** по наличию sidecar
   ...
   есть и **валиден по схеме кита целиком** — структура вердикта плюс схема
   каждой находки: `apply-threshold.sh` отвергает схемно негодный вердикт кодом
   2, поэтому структурно годный, но схемно негодный sidecar даёт
   `invalid_verdict`, а не `verdict`); `guardrail_rejection` (код 2 с текстом потолка
   дифа — ожидаемо для `large` без `local_args`); `invalid_verdict` (sidecar
   есть, но вердикт **негоден по схеме**: код 2 от `apply-threshold.sh`, чьи
   правила строже схемы — например `kind: file-missing` с `line > 0`, — либо
   код 0/1 при вердикте, не проходящем схему) — это ошибка **модели**, не
   конфигурации; `config_failure` (код 2 без sidecar **либо** код 2 при
   схемно годном вердикте: sidecar пишется до порога, значит порог отказал по
   своей причине — нет `jq`, негодный аргумент, — и списывать это на модель
   нельзя); `mechanical_failure`
   ...
   Отдельно — `empty_range`: `base_sha` и `head_sha` кейса дают **пустой диф**
   (разные коммиты с одинаковым деревом: правка и её откат). Раннер проверяет
   диапазон `merge-base(base, head)..head` — тот же, что берёт `local.sh` — в
   worktree, офлайн и **до** вызова ревьюера, и такой прогон записывается с
   `reviewer_ran: false`, без sidecar и стоимости. Прежде кит выходил на
   пустом дифе кодом 0 без sidecar, и исход считался `mechanical_failure`:
   негодный **кейс** выглядел сбоем инструмента. `empty_range` не годен для
   метрик качества и попадает в счётчик `empty_range_count`.
...
Возобновление в тот же `--out` ограничено правилами про манифест.
`repetitions` **только растут**, и `--rerun` от этого не освобождает: меньшее
число перезаписало бы `run.json`, оставив на диске `result.json` старших
повторений (`--rerun` сносит лишь 1..N нового N), и метрики посчитались бы по
прогонам, которых манифест не объявляет; выход — новый `--out`. Менять
`repetitions` разрешено только запросом по **полному** набору кейсов манифеста:
доливка выборки с другим числом давала манифест, объявляющий старшие
повторения у всех кейсов, тогда как появлялись они лишь у выбранных.
`load_results` вдобавок требует **полноты**: манифест объявляет произведение
«кейсы × варианты × 1..repetitions», и отсутствующий `result.json` — отказ с
перечнем троек. Прогон в процессе или оборвавшийся просто не отчётен: `status:
ok` по половине прогона выглядел бы измерением. И `load_results` отказывается читать
результат вне манифеста — повторение больше объявленного, метка варианта или
`case_id`, в манифесте отсутствующие: тихо включить его в метрики нельзя, тихо
выбросить — тоже (исход оплачен). Набор кейсов, как и кит с корпусом, входит в
проверку провенанса, но по правилу **вхождения**: запрошенная выборка обязана
лежать внутри списка манифеста (равенство допустимо), выход за него — отказ,
потому что прогон по A и прогон по A+B разные измерения, и один `run.json` их
не описывает. Выборка список манифеста **не сжимает**: он описывает прогон
целиком, а `--rerun --cases <подмножество>` переизмеряет только выборку,
оставляя результаты остальных кейсов на месте. Выход из отказов один: `--rerun`
по полному набору кейсов либо новый `--out`.

`load_results` вдобавок сверяет **путь с содержимым**: артефакт называет тройку
дважды — деревом `cases/<case_id>/<variant>/<rep>/` и полями внутри
`result.json`, — и расхождение это отказ, а не выбор, какому утверждению
верить. Каждая тройка обязана встретиться ровно один раз: копия результата под
другим именем каталога удвоила бы прогон в знаменателях, и сообщение называет
оба пути.

**Версии инструментов** (`tools`) входят в проверку провенанса: клиенты
используемых харнессов (харнесс `codex` → версия `codex`, `claude` → версия
`claude`) **и `git`**. Клиенты — это измеряемый ревьюер снаружи кита, а `git`
формирует сам диф: `local.sh` выбирает алгоритм его подготовки по возможностям
git (`check-attr --source`), то есть обновление git меняет вход ревьюера.
Доливка после обновления давала половину результатов от прежней версии под
манифестом от новой; обновили — новый `--out` либо полный `--rerun`. Переход
между `unavailable` и настоящей версией — такой же дрейф. Из сверки исключены
только **неиспользуемые** клиенты ревьюера: они пишутся в `run.json` для
протокола, но на вход прогона не влияют.

**Значения provider-переменных** сверяются отпечатком
`provider_env_fingerprint` — sha256 строк `имя=значение` тех
provider/proxy-переменных, чьё имя не выглядит секретом
(`KEY`/`TOKEN`/`SECRET`/`PASSWORD`/`CREDENTIAL`, без учёта регистра). Одних
имён мало: смена значения `HTTPS_PROXY` набор имён не меняет, и половина
прогона могла уйти другим маршрутом под одним манифестом. Значения секретных по
имени переменных в отпечаток **не входят** — манифест публикуется (§10), а
дайджест ключа провайдера вскрывается словарём, и соль в том же файле не
помогла бы. Принятый предел: смена ключа при том же имени переменной для
дрейфа невидима.

**Незавершённый прогон не читается.** `run.json` пишется дважды — в начале с
`finished: null`, в конце целиком, — поэтому полный набор `result.json` при
незакрытом манифесте значит обрыв между последней тройкой и финальной записью,
а не готовый прогон: `load_results` отказывает.

**Результаты без манифеста** и `--rerun` частью: перезапуск допустим лишь
когда сброс накрывает **все** существующие результаты. Иначе `run.json`
объявил бы своими и те, чей провенанс неизвестен.

**Симлинков в каталоге прогона не бывает.** Каталог тройки проверяется на
симлинки по всему участку от `--out`, и каждый лист-артефакт
(`verdict.json`, `usage.json`, `result.json`, логи) — перед созданием, удалением
и записью: ссылка на месте артефакта уводила удаление и запись кита за пределы
прогона. Путь листа при этом **не резолвится** — резолвинг и был дырой. То же
правило на чтении: «готовый результат» — обычный файл без симлинков на участке
от `--out`, поэтому подложенная ссылка не пропускает тройку как готовую и не
читается `load_results`.

**Варианты прогона не повторяются**: метка варианта — имя каталога артефактов,
поэтому дубликат писал бы артефакты по одному пути из двух задач (при
`--jobs > 1` одновременно), а счёт прогонов вырос бы вдвое без второго
измерения. Дубликат — отказ до любой работы.

**Версии инструментов резолвятся в окружении прогона**, а не процесса: раннер
...
- `completion_rate` — исходы `verdict` (или ожидаемый `guardrail_rejection`) /
  ...
  все предсказания.
- `empty_range_count` — счётчик (знаменатель — все прогоны варианта): прогоны с
  исходом `empty_range`, то есть кейсы, у которых ревьюировать нечего. Это
  проблема **корпуса**, а не инструмента, поэтому счётчик отдельный: в
  `mechanical_failure_rate` такой прогон не входит, в метрики качества — тоже.
  `report.md` печатает его в эксплуатационном блоке.
- `refuted_file_missing_count` — счётчик (не доля; знаменатель — все
  предсказания прогонов, где матчер отработал): находки `kind: file-missing`,
  **опровергнутые деревом** `head_sha` — файл, которого «нет», на самом деле
  есть. Матчер таким находкам рёбер к строчному gold не даёт вовсе (это разные
  утверждения), поэтому без опровержения они уходили бы в очередь
  adjudication — а разбирать там нечего: дерево head и есть ответ.
  Опровергнутая находка считается FP в `precision_lower_bound`, в очередь не
  попадает и официальный `precision` не держит. Источник фактов о файлах тот
  же, что у `resolvable_evidence_rate`; нет источника (`CacheUnavailable`) —
  нет и опровержений, находка остаётся в очереди. Опровержение **не зависит от
  назначения**: матчер дерева не читает, и назначенная gold-дефекту
  `file-missing` при существующем файле тоже ложна — назначение снимается.
- `contradicted_gold_count` — счётчик (знаменатель — gold-дефекты прогонов, где
  матчер отработал): gold-дефекты, которым была назначена опровергнутая
  находка. Gold утверждает «файла нет», дерево `head_sha` говорит обратное,
  значит неверна **разметка кейса**, а не предсказание. Такой дефект не даёт TP,
  исключён из знаменателей `blocking_recall` и
  `detection_recall_any_severity` (требовать найти отсутствующий в материале
  дефект — наказывать вариант за правильное поведение), печатается в
  `adjudication-queue.md` отдельным разделом «gold противоречит дереву head» и,
  пока он есть, держит вариант вне `status: ok` (та же механика, что у непустой
  очереди: `precision` не публикуется, статус `pending_adjudication`).
  Считается по **всем** прогонам, где отработал матчер, а не только по годным
  для метрик качества: разметка спорит с деревом независимо от того, каким
  вышел исход прогона, в котором это заметили. Иначе противоречие, найденное в
  прогоне с неожидаемым исходом, оставляло вариант в `ok`, а сводку очереди —
  со словами «очередь пуста».

  Проверяется **каждый** gold-дефект `kind: file-missing`, независимо от
  находок варианта: `file_lines(defect.file)` не `None` — дефект противоречит
  дереву. Путь передаётся **сырым**, без предварительного `normalize_path`:
  нормализация — обязанность самой реализации `file_lines` (сырой путь
  сперва, нормализованный — запасной вариант), не предусловие вызова; иначе
  предварительная нормализация уничтожала бы литеральный символ настоящего
  git-имени раньше, чем до него доходил сырой поиск.

  Прежде противоречие замечалось только через
  назначенную опровергнутую находку, то есть требовало, чтобы модель повторила
  то же ложное утверждение, и молчаливая модель оставляла негодный gold в
  знаменателе recall. Сторона предсказаний осталась как второй путь. Без
  источника фактов о файлах (`CacheUnavailable`) сверка gold не выполняется —
  об этом говорит пометка `resolvable_evidence_rate` того же прогона.
...
eval/runs/<run_id>/
  run.json          # kit: {commit, prompt_sha256, schema_sha256, threshold_sha256,
                    ...
                    # provider_env_names + provider_env_fingerprint: имена
                    #   provider/proxy-переменных и sha256 их значений без секретных
                    #   (KEY/TOKEN/SECRET/PASSWORD/CREDENTIAL — только имя);
                    # cases: [case_id, …] — отсортированные кейсы прогона: дайджест
                    #   отвечает «тот ли корпус», список — «какие кейсы измеряли»,
                    #   и по нему же `load_results` отвергает результат чужого кейса;
