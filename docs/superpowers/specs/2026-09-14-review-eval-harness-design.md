...
**Версии инструментов** (`tools`) входят в проверку провенанса — но только у
тех клиентов, которыми пользуются варианты прогона: харнесс `codex` → версия
`codex`, харнесс `claude` → версия `claude`. Они и есть измеряемый ревьюер
снаружи кита, и доливка после их обновления давала половину результатов от
прежней версии под манифестом от новой; обновили используемый клиент — новый
`--out` либо полный `--rerun`. Переход используемого клиента между
`unavailable` и настоящей версией — такой же дрейф. `git` и неиспользуемые
клиенты пишутся в `run.json` для протокола, но в сверку не входят: диапазон
задаёт раннер по `base_sha`/`head_sha`, материал берётся из bare-кэша, а
клиент, которым прогон не пользуется, к измеренному отношения не имеет.
...
eval/runs/<run_id>/
  run.json          # kit: {commit, prompt_sha256, schema_sha256, threshold_sha256,
                    ...
                    #   только клиенты используемых харнессов, git и неиспользуемые
