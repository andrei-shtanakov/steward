...
**Версии инструментов** (`tools`) входят в проверку провенанса наравне с китом:
версия `claude`/`codex` — это измеряемый ревьюер снаружи кита, а версия `git`
определяет диф. Доливка после обновления CLI давала половину результатов от
прежней версии под манифестом от новой. Обновили инструмент — новый `--out`
либо полный `--rerun`.
