...
бы из такой строки дырку в правиле. Слаг репо для `case_id` и id — тот же
`owner.name` (§5 выше: нижний регистр, `.` и `_` имени сохраняются, owner по
форме GitHub без точек), так что `org/my_repo#7` → `org.my_repo-7`;
другого правила нормализации нет. Синтаксис проверяется у **каждой**
