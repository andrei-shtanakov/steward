...
**Отдельно посмотреть `arbiter` и `atp-platform`:** они на релизе кита до
адаптера, и цикл пересчёта PIN там упадёт на отсутствующем члене. Их либо
подтягивать к текущему релизу отдельно, либо вести отдельной формой волны —
решение владельца, не механика.
