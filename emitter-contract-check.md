...
## Addendum (2026-07-15): находка закрыта с обеих сторон

- **Maestro** исправил баг 2026-07-06 (PR #47): `_check_dangling_deps` в
  `maestro/preflight.py` флагует висячую `depends_on` ошибкой `dangling-dep`; проверка
  выполняется до `check_fs`-ветки, т.е. срабатывает и в `validate --no-fs`. Тогда же
  закрыт C4 (PR #46 + #50): decomposer делегирует генерацию спек в `spec-runner plan
  --full`, встроенного дубля формата нет.
- **steward** проверяет целостность `depends_on` сам — `GC-COMPILE`
  (`src/steward/gatecheck/checks.py::check_compile_block`) валидирует нормализованный
  `yaml steward-compile` блок до компиляции. Остаётся как defense-in-depth: ловит битую
  ссылку на слое governance, раньше и с более точной диагностикой.
