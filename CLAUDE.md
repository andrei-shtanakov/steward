...
- `uv run gate-check --profile team spec/` — run the gate-check CLI (`--arch-stage authoring|release` selects the GC-ARCH-CONFORMANCE stage policy, `profiles/arch-policy.yaml`; default `authoring`)
...
- Two prograph contracts are vendored this way: `contracts/prograph-intended-graph/v1/schema.json` and `contracts/prograph-conformance-report/v1/schema.json`. Each carries **two separate guarantees**, never conflated: copy-integrity (the vendored file is byte-identical to its producer's, checked offline in this repo's own CI) and upstream-drift (whether prograph's schema has since moved, watched on a schedule outside this repo's gate). steward is a read-only, offline consumer of prograph's conformance evidence — it never regenerates a report itself.
