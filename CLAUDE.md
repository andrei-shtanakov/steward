...
- `uv run gate-check --profile team --candidate --upto <node> <dir>` — judge an incomplete bundle up to `<node>`'s DAG level (steward#187): only completeness is relaxed above it, the boundary is declared (`upto` in JSON); incompatible with `--stage release` / `--emit-verdicts`. Docs: `docs/gate-check-candidate.md`
