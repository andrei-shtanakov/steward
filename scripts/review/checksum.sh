...
while IFS= read -r line || [ -n "$line" ]; do
    ...
    # Симлинк на месте члена кита — отказ ЦЕЛОСТНОСТИ, даже при совпадающих
    # байтах цели: `-f` разыменовывает, и структурно подменённый кит
    # проходил бы зелёным (major четвёртого захода). Тот же класс, что
    # «обычный файл, не симлинк» при извлечении механики из base в CI.
    if [ -L "$root/$path" ]; then
        echo "СИМЛИНК вместо файла: $path — вендор-копия обязана быть" \
            "обычным файлом." >&2
        failed=$((failed + 1))
        continue
    fi
