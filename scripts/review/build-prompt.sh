...
# пропущенного lock'а). Доказательство берётся из рабочего дерева, из корня
# которого идёт прогон, — в CI это чекаут merge-ref PR, локально сам репо,
# то есть дерево совпадает с тем, к которому относится диф:
