...
#
# Список — известные lock-файлы и снапшоты, узкий намеренно: ложное опущение
# исходника хуже пропущенного lock'а. Расширение списка — правка кита через
# ревью.
# Фильтр включается ТОЛЬКО когда в дифе есть generated-блоки: awk на выходе
# нормализует хвостовой перевод строки, и прогон без generated-файлов получил
# бы другой маркер и не-побайтовый диф — свойство «без опций вывод неизменен»
...
awk '
    /^diff --git / {
        path = $NF; sub(/^b\//, "", path)
        base = path; sub(/.*\//, "", base)
        generated = (base == "uv.lock" || base == "package-lock.json" \
            || base == "yarn.lock" || base == "pnpm-lock.yaml" \
            || base == "poetry.lock" || base == "Cargo.lock" || base == "go.sum" \
            || base ~ /\.snap$/)
        if (generated) {
