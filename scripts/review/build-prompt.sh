...
# То же и для withheld-списка, и по той же причине, только цена другая: у
# generated-фильтра пропажа списка означала бы «фильтр не применён», здесь —
# «объявление об удержанном не напечатано». Промпт при этом соберётся
