...
# Та же феатур-детекция, что у --generated-list ниже и по той же причине:
# при перекосе версий копий (новый local.sh, старый build-prompt.sh) слепая
# передача флага убила бы прогон кодом 2. Нет поддержки — маркер не уходит,
# и это объявляется вслух: молчаливая потеря объявления вернула бы ровно тот
# дефект, который steward#176 и починил.
if [ -s "$work/withheld.list" ]; then
    if grep -q -- '--withheld-list' "$kit_dir/build-prompt.sh"; then
        set -- "$@" --withheld-list "$work/withheld.list"
    else
        info "build-prompt.sh этой копии кита не знает --withheld-list:" \
            "модель не узнает об удержанных путях — ре-вендорьте кит"
    fi
fi
