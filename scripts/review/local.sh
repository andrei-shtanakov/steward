...
'
    ...
    for _f in $_changed_paths; do
        IFS=$_old_ifs
        set +f
        path_is_prose "$_f" || set -- "$@" ":(top,literal)$_f"
        IFS='
'
        set -f
    done
    ...
    # Fail-closed страж класса целиком (C-1 финального ревью, вторая
    # обязательная половина правки): выше уже проверено `$# -gt 0` —
    # классификация нашла хотя бы один НЕ-прозаический путь (иначе уже был
    # бы `exit 5`). Значит непустой `diff.patch` здесь ГАРАНТИРОВАН, если
    # классификация и pathspec согласованы. Пустой результат — не «находок
    # нет», а сломанный инструмент: pathspec не совпал ни с одним реальным
    # путём. Эта проверка не привязана к конкретной причине рассогласования
    # (квотирование — только ОДНА уже найденная форма) — она ловит класс
    # целиком, включая ещё не найденные его формы, тем же кодом 3, что и
    # прочие механические отказы этого блока.
    if [ ! -s "$work/diff.patch" ]; then
