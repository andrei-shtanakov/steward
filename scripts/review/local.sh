...
#
# `base_looks_remote` — синтаксическая проверка ПО ТЕКСТУ `$base`, без
# обращения к git: полная форма (`refs/remotes/...`) или что угодно с `/`
# (шортхенд вида `<remote>/<ветка>`, возможно ЕЩЁ не подтянутый локально —
# резолвить его здесь нельзя, поэтому гадаем по синтаксису, не по факту
# существования). Ложноположительный результат безвреден: он просто пускает
# remote-резолюцию, которая и так no-op на уже настроенном remote'е.
remote_needed=0
