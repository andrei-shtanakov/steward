...
if [ "$trusted_base_explicit" -eq 1 ]; then
    ...
    # это не симметрия ради симметрии: направление отказа здесь —
    # в сторону СОКРЫТИЯ. Протухший локально `origin/master`, с которого
    # в base уже сняли `dist/* linguist-generated`, заставит кит читать
    # СТАРУЮ декларацию, вето не включится (ветка `.gitattributes` не
    # трогала), и рукописный код уедет под маркер опущения. Безусловный
    # дисклеймер «кит не проверяет» этого не различал: он печатался и на
    # свежей границе тоже (находка minor круга 3 — асимметрия с активной
    # проверкой `$base`).
    #
    # Remote НЕ УГАДЫВАЕТСЯ: если `$trusted_base` — remote-tracking ref,
    # его полное имя (`refs/remotes/<remote>/<ветка>`) само называет и
    # remote, и ветку. Поэтому проверка работает и там, где `$remote`
    # к границе отношения не имеет: при `--base <sha> --trusted-base …`
    # блок резолва remote'а выше вообще не исполнялся.
    #
    # Только предупреждение, без `--fetch`: тянуть чужую ревизию кит не
    # станет — что и когда фетчить, знает вызывающий, он границу и назвал.
    # В fp-режиме проверки нет вовсе (ls-remote — сеть), ровно как у базы:
    # устаревшая граница даст честно другой отпечаток.
    trusted_full=""
    if [ "$fp_only" -eq 0 ]; then
        trusted_full=$(git rev-parse --symbolic-full-name "$trusted_base" \
            2>/dev/null) || trusted_full=""
    fi
    case "$trusted_full" in
        refs/remotes/*/*)
            _tb_rest="${trusted_full#refs/remotes/}"
            _tb_remote="${_tb_rest%%/*}"
            _tb_branch="${_tb_rest#*/}"
            if ! _tb_line=$(git ls-remote --heads "$_tb_remote" "$_tb_branch" \
                2>/dev/null); then
                info "  свежесть доверенной базы не проверена (нет связи с $_tb_remote)"
            elif [ -z "$_tb_line" ]; then
                info "  ВНИМАНИЕ: ветки '$_tb_branch' нет на $_tb_remote —" \
                    "граница доверия указывает на устаревшую ссылку"
            else
                _tb_remote_sha=$(printf '%s' "$_tb_line" | cut -f1)
                _tb_local_sha=$(git rev-parse "$trusted_base")
                if [ "$_tb_remote_sha" != "$_tb_local_sha" ]; then
                    _tb_behind=$(git rev-list --count \
                        "$trusted_base..$_tb_remote_sha" 2>/dev/null || echo "?")
                    info "  ВНИМАНИЕ: доверенная база устарела на" \
                        "$_tb_behind коммит(ов) против $_tb_remote/$_tb_branch —" \
                        "снятые с тех пор декларации ещё действуют;" \
                        "выполните: git fetch $_tb_remote $_tb_branch"
                else
                    info "  свежесть доверенной базы: совпадает с $_tb_remote/$_tb_branch"
                fi
            fi
            ;;
        *)
            info "  свежесть доверенной базы не проверяется:" \
                "она не remote-tracking ref (голый SHA, локальная ветка" \
                "или fp-режим) — актуальность на вызывающем"
            ;;
    esac
