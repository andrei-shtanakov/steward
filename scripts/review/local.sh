...
# "duplicate"/"empty" — отказ независимо от вызывающего, их текст $4 не
# трогает.
read_scope_key() {
    _rsk_key="$1"
    ...
    case "$_rsk_count" in
        ...
        *)
            info "правило области ревью ($3): ключ $_rsk_key определён" \
                "$_rsk_count раз — файл битый, фильтр не применяется;" \
    ...
    if [ -z "$scope_key_value" ]; then
        info "правило области ревью ($3): пустое значение $_rsk_key —" \
...
if [ "$include_prose" -eq 1 ]; then
    ...
    if read_scope_key PROSE "$_scope_lines" "$scope_rules"; then
        ...
        if read_scope_key CODE_OVERRIDE "$_scope_lines" "$scope_rules"; then
