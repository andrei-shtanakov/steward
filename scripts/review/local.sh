...
# Печатается только при явном флаге: без него граница совпадает с базой, и
# отдельная строка была бы шумом в каждом прогоне флота. А вот РАЗВЕДЁННЫЕ
# диапазон и граница оператор обязан видеть — иначе "почему этот файл
# показан целиком" не восстановить по выводу.
