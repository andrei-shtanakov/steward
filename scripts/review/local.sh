...
#
# $1 — ключ (PROSE|CODE_OVERRIDE), $2 — содержимое файла правила ПОСЛЕ
# обрезки пробелов по всей строке и удаления комментариев/пустых строк (см.
# вызов ниже), $3 — путь к файлу (для сообщений). Тот же контракт, что у
# read_scope_key в devtools/review-pr.sh:560-586 (референс среза A, спека
# §4.3): один ключ — одна непустая строка; ноль определений, больше одного,
# пустое значение — КАЖДОЕ из трёх делает правило негодным. Разница с
# обвязкой принципиальна: там `die 2` (правило — конфигурация ВЫЗЫВАЮЩЕГО),
# здесь fail-open в сторону ревью (§4.3: неразбираемое правило — полный
# диф, а не половина правила) — при отказе пишет причину через `info()` и
# возвращает 1, `scope_key_value` остаётся пустым; падать самому нельзя.
