...
if scope_cfg=$(git show "$mb:$scope_cfg_path" 2>/dev/null); then
    ...
    if read_scope_key PROSE_REVIEW "$_scope_cfg_lines" "$scope_cfg_path"; then
...
if [ -n "$prose_globs" ]; then
    ...
    if ! _changed_paths=$(git -c core.quotePath=false diff --no-renames \
            --name-only "$mb..$head_sha"); then
    ...
    # диапазоне, которого модель не видела (Critical C-1 ревью этой ветки).
    # `:(top,...)` фиксирует базу в корне репо независимо от CWD.
    # --no-renames: и старый, и новый путь переименования проходят
    # классификацию. `core.quotePath=false` — тот же довод, что и у
    # соседнего вызова ниже (@id:review-kit-quoted-diff-headers): сырые
    # пути, не C-style-квотированные для non-ASCII/спецсимволов, иначе
    # `path_is_prose`/`:(top,literal)` сравнивали бы классификатор с
    # квотированной строкой, а не с реальным путём файла.
    _old_ifs=$IFS
    IFS='
'
    set -f
