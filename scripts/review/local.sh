...
# работы, а не после — устаревший вердикт не должен пережить отказ, чем бы
# отказ ни был вызван (пустой диф, пустой остаток после фильтра, падение
# ревьюера). Но стирать имеет право только прогон, который МОГ БЫ ревью
# провести: режимы-запросы (`--print-review-cmd` выше, `--fingerprint-only`
# ниже) ревьюера не зовут и вердикта не производят — снести чужой файл ради
# ответа на вопрос было бы чистой потерей данных. Валидация формы
# (пустое значение, каталог вместо файла) остаётся в префлайте и работает
# во всех режимах; сюда вынесен только `rm -f`.
