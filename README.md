...
Удаление ветки и пуш тега хук блокирует по построению, а не из-за поломки:
ревьюировать в обоих случаях нечего, а «проверили одно, отправили другое» он
не допускает. В частности, штатная чистка влитой ветки после мержа
(`git push origin --delete <ветка>` из `CLAUDE.md`) упрётся в блокировку —
способ тот же: `git push --no-verify --delete <ветка>`.
