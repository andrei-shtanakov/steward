...
`scripts/review/harness-claude` — член кита (в этом релизе переходный —
незапинованный файл адаптера у потребителя — отказ copy-integrity, а не
пропуск; в обязательный член адаптер переводится следующим релизом кита), и
